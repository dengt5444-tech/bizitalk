...
ストックフォト(実在の人物を想起させる写真)は使わず、ブランドカラーで統一した抽象的なグラデーションイラスト(`src/components/illustrations/`)をホームページや会話シーン一覧に使用しています。`HeroIllustration` はトップページの「会話中」を表す抽象コンポジション、`CategoryIllustration` は4カテゴリー(同僚・海外の同僚・顧客・経営陣)それぞれに固有のグラデーションとアイコンを持たせたバッジで、どの画面でも同じ見た目で使い回せます。
