...
export async function listScenarios(): Promise<ScenarioSummary[]> {
  const { data, error } = await supabase
    ...
    .select("id, slug, title, description, category, level, persona_name, persona_role, is_free, order_index")
...
export async function getScenarioBySlug(slug: string): Promise<ScenarioDetail | null> {
  const { data, error } = await supabase
    .from("conversation_scenarios")
    .select(
      "id, slug, title, description, category, level, persona_name, persona_role, persona_background, opening_line, is_free, order_index",
    )
    .eq("slug", slug)
    .maybeSingle();
